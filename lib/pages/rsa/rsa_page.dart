...
class _RsaPageState extends State<RsaPage> {
  ...
  void _loadRsaKeys() async {
    ...
    if (publicKey != null && privateKey != null) {
      if (context.mounted) {
        context.read<RsaProvider>().updatePublicKey(publicKey);
        context.read<RsaProvider>().updatePrivateKey(privateKey);
      } else {
        if (kDebugMode) {
          setState(() {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<RsaProvider>(
      create: (context) => RsaProvider(),
      child: Scaffold(
        appBar: AppBar(
        ...
        body: InitialBody(
          child: Column(
            children: [
              ...
              context.watch<RsaProvider>().publicKey == null &&
                      context.watch<RsaProvider>().privateKey == null
                  ? CustomButton(
                      title: 'Gerar chave',
                      onPressed: () {
                        _generateKeys();
