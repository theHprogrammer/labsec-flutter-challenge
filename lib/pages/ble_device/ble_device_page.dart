...
import '/widgets/box/ble_box/export_ble_box.dart';
...
class _BleDevicePageState extends State<BleDevicePage> {
  ...
  Future<void> _addDeviceTolist(final BluetoothDevice device) async {
    try {
      if (context.mounted) {
        ...
        if (!context.read<BleDeviceProvider>().devicesList.contains(device)) {
          ...
          if (device.name == null || device.name == '') {
            context.read<BleDeviceProvider>().nameDevices.add('Unknow Device');
          } else {
            context.read<BleDeviceProvider>().nameDevices.add(device.name);
          }
          context.read<BleDeviceProvider>().devicesList.add(device);
          context.read<BleDeviceProvider>().idDevices.add(device.id.id);
        }
      }
    } catch (e) {
      // Se ocorrer algum erro, apresenta uma mensagem de erro.
  ...
  Future<void> _scanBle() async {
    try {
      ...
      if (context.read<BleDeviceProvider>().devicesList != null ||
          context.read<BleDeviceProvider>().devicesList.isNotEmpty) {
        // Limpa a lista de dispositivos BLE e muda o deviceScan para "Escaneando...".
        setState(() {
          context.read<BleDeviceProvider>().devicesList.clear();
          context.read<BleDeviceProvider>().nameDevices.clear();
          context.read<BleDeviceProvider>().idDevices.clear();
        ...
        if (context.mounted) {
          ...
          if (context.read<BleDeviceProvider>().devicesList.isNotEmpty) {
            SchedulerBinding.instance.addPostFrameCallback((_) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content:
          ...
          if (context.mounted) {
            ...
            context
                .read<BleDeviceProvider>()
                .updateIdDevices(context.read<BleDeviceProvider>().idDevices);
            context.read<BleDeviceProvider>().updateNameDevices(
                context.read<BleDeviceProvider>().nameDevices);
            context.read<BleDeviceProvider>().updateDevicesList(
                context.read<BleDeviceProvider>().devicesList);
          }
        }
      }
      // Se ocorrer algum erro, apresenta uma mensagem de erro.
  ...
  Future<void> _loadListBle() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      context.read<BleDeviceProvider>().updateIdDevices(
          prefs.getStringList('idDevices')?.map((e) => e.toString()).toList() ??
              <String>[]);
      context.read<BleDeviceProvider>().updateNameDevices(prefs
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ChangeNotifierProvider<BleDeviceProvider>(
            ...
            return InitialBody(
              child: Column(
                ...
                children: [
                  BleBox(
                    dateTime: dateTime,
                    devicesList: context.read<BleDeviceProvider>().toJson(
                        context.watch<BleDeviceProvider>().nameDevices,
                        context.watch<BleDeviceProvider>().idDevices),
